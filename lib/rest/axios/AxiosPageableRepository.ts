...
export default class AxiosPageableRepository extends AxiosRepository
  implements PageableRepository {
  public nextPage(page: Page<any>): Promise<Page<any>> {
    if (page.last) {
  ...
  public previousPage(page: Page<any>): Promise<Page<any>> {
    if (page.first) {
  ...
  public showPage(request: PageRequest = 1): Promise<Page<any>> {
    let params: any = {}
